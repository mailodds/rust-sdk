...
/// struct for typed errors of method [`get_mcp_capabilities`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetMcpCapabilitiesError {
...
pub async fn get_mcp_capabilities(configuration: &configuration::Configuration, ) -> Result<models::McpCapabilities, Error<GetMcpCapabilitiesError>> {

    let uri_str = format!("{}/v1/mcp/capabilities", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::McpCapabilities`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::McpCapabilities`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetMcpCapabilitiesError> = serde_json::from_str(&content).ok();
