...
/// struct for typed errors of method [`classify_content`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ClassifyContentError {
...
pub async fn classify_content(configuration: &configuration::Configuration, classify_content_request: models::ClassifyContentRequest) -> Result<models::ClassifyContent200Response, Error<ClassifyContentError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_classify_content_request = classify_content_request;

    let uri_str = format!("{}/v1/content-check", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_classify_content_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ClassifyContent200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ClassifyContent200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ClassifyContentError> = serde_json::from_str(&content).ok();
