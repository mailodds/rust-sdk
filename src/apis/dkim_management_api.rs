...
/// struct for typed errors of method [`get_dkim_dns_record`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetDkimDnsRecordError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`rotate_dkim`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum RotateDkimError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_dkim_dns_record(configuration: &configuration::Configuration, domain_id: &str) -> Result<(), Error<GetDkimDnsRecordError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_domain_id = domain_id;

    let uri_str = format!("{}/v1/sending-domains/{domain_id}/dkim/dns-record", configuration.base_path, domain_id=crate::apis::urlencode(p_path_domain_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<GetDkimDnsRecordError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Generate a new DKIM key pair and rotate the selector for a sending domain.
pub async fn rotate_dkim(configuration: &configuration::Configuration, domain_id: &str) -> Result<(), Error<RotateDkimError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_domain_id = domain_id;

    let uri_str = format!("{}/v1/sending-domains/{domain_id}/dkim/rotate", configuration.base_path, domain_id=crate::apis::urlencode(p_path_domain_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<RotateDkimError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
