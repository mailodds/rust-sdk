...
/// struct for typed errors of method [`track_event`]
...
pub async fn track_event(configuration: &configuration::Configuration, track_event_request: models::TrackEventRequest) -> Result<models::TrackEventResponse, Error<TrackEventError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_track_event_request = track_event_request;

    let uri_str = format!("{}/v1/events/track", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_track_event_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::TrackEventResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::TrackEventResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<TrackEventError> = serde_json::from_str(&content).ok();
