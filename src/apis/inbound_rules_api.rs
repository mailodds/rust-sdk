...
/// struct for typed errors of method [`create_inbound_rule`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreateInboundRuleError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`delete_inbound_rule`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum DeleteInboundRuleError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`dry_run_inbound_rules`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum DryRunInboundRulesError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`get_inbound_rule`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetInboundRuleError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`list_inbound_rules`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListInboundRulesError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`update_inbound_rule`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum UpdateInboundRuleError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}
...
pub async fn create_inbound_rule(configuration: &configuration::Configuration, domain_id: &str) -> Result<(), Error<CreateInboundRuleError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_domain_id = domain_id;

    let uri_str = format!("{}/v1/sending-domains/{domain_id}/inbound-rules", configuration.base_path, domain_id=crate::apis::urlencode(p_path_domain_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<CreateInboundRuleError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Delete an inbound routing rule.
pub async fn delete_inbound_rule(configuration: &configuration::Configuration, domain_id: &str, rule_id: &str) -> Result<(), Error<DeleteInboundRuleError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_domain_id = domain_id;
    let p_path_rule_id = rule_id;

    let uri_str = format!("{}/v1/sending-domains/{domain_id}/inbound-rules/{rule_id}", configuration.base_path, domain_id=crate::apis::urlencode(p_path_domain_id), rule_id=crate::apis::urlencode(p_path_rule_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<DeleteInboundRuleError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Evaluate inbound rules against a test message without side effects.
pub async fn dry_run_inbound_rules(configuration: &configuration::Configuration, domain_id: &str) -> Result<(), Error<DryRunInboundRulesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_domain_id = domain_id;

    let uri_str = format!("{}/v1/sending-domains/{domain_id}/inbound-rules/dry-run", configuration.base_path, domain_id=crate::apis::urlencode(p_path_domain_id));
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<DryRunInboundRulesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Retrieve a specific inbound routing rule by ID.
pub async fn get_inbound_rule(configuration: &configuration::Configuration, domain_id: &str, rule_id: &str) -> Result<(), Error<GetInboundRuleError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_domain_id = domain_id;
    let p_path_rule_id = rule_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetInboundRuleError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// List all inbound routing rules for a sending domain.
pub async fn list_inbound_rules(configuration: &configuration::Configuration, domain_id: &str) -> Result<(), Error<ListInboundRulesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_domain_id = domain_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ListInboundRulesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Update an existing inbound routing rule.
pub async fn update_inbound_rule(configuration: &configuration::Configuration, domain_id: &str, rule_id: &str) -> Result<(), Error<UpdateInboundRuleError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_domain_id = domain_id;
    let p_path_rule_id = rule_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<UpdateInboundRuleError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
