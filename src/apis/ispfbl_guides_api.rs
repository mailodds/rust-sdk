...
/// struct for typed errors of method [`get_isp_fbl_guide`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetIspFblGuideError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`list_isp_fbl_guides`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListIspFblGuidesError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_isp_fbl_guide(configuration: &configuration::Configuration, isp_id: &str) -> Result<(), Error<GetIspFblGuideError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_isp_id = isp_id;

    let uri_str = format!("{}/v1/isp-fbl/guides/{isp_id}", configuration.base_path, isp_id=crate::apis::urlencode(p_path_isp_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<GetIspFblGuideError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// List all ISP feedback loop setup guides.
pub async fn list_isp_fbl_guides(configuration: &configuration::Configuration, ) -> Result<(), Error<ListIspFblGuidesError>> {

    let uri_str = format!("{}/v1/isp-fbl/guides", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ListIspFblGuidesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
