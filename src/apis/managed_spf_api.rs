...
/// struct for typed errors of method [`create_managed_spf`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreateManagedSpfError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`get_managed_spf`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetManagedSpfError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`refresh_managed_spf`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum RefreshManagedSpfError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`update_managed_spf`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum UpdateManagedSpfError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}
...
pub async fn create_managed_spf(configuration: &configuration::Configuration, domain_id: &str) -> Result<(), Error<CreateManagedSpfError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_domain_id = domain_id;

    let uri_str = format!("{}/v1/sending-domains/{domain_id}/managed-spf", configuration.base_path, domain_id=crate::apis::urlencode(p_path_domain_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<CreateManagedSpfError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Retrieve the managed SPF record for a sending domain.
pub async fn get_managed_spf(configuration: &configuration::Configuration, domain_id: &str) -> Result<(), Error<GetManagedSpfError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_domain_id = domain_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetManagedSpfError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Re-resolve DNS and refresh the managed SPF record for a sending domain.
pub async fn refresh_managed_spf(configuration: &configuration::Configuration, domain_id: &str) -> Result<(), Error<RefreshManagedSpfError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_domain_id = domain_id;

    let uri_str = format!("{}/v1/sending-domains/{domain_id}/managed-spf/refresh", configuration.base_path, domain_id=crate::apis::urlencode(p_path_domain_id));
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<RefreshManagedSpfError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Update managed SPF settings such as enabling or disabling for a sending domain.
pub async fn update_managed_spf(configuration: &configuration::Configuration, domain_id: &str) -> Result<(), Error<UpdateManagedSpfError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_domain_id = domain_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<UpdateManagedSpfError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
