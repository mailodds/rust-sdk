...
/// struct for typed errors of method [`get_message_events`]
...
pub async fn get_message_events(configuration: &configuration::Configuration, message_id: &str) -> Result<models::GetMessageEvents200Response, Error<GetMessageEventsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_message_id = message_id;

    let uri_str = format!("{}/v1/message-events", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetMessageEvents200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetMessageEvents200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetMessageEventsError> = serde_json::from_str(&content).ok();
