...
/// struct for typed errors of method [`oauth_register_client`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum OauthRegisterClientError {
    UnknownValue(serde_json::Value),
}
...
pub async fn oauth_register_client(configuration: &configuration::Configuration, ) -> Result<models::OAuthClientRegistration, Error<OauthRegisterClientError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::OAuthClientRegistration`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::OAuthClientRegistration`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<OauthRegisterClientError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
