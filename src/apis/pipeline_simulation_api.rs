...
/// struct for typed errors of method [`simulate_pipeline`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum SimulatePipelineError {
...
pub async fn simulate_pipeline(configuration: &configuration::Configuration, ) -> Result<(), Error<SimulatePipelineError>> {

    let uri_str = format!("{}/v1/simulate", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<SimulatePipelineError> = serde_json::from_str(&content).ok();
