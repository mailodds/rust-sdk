...
/// struct for typed errors of method [`get_pixel_settings`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetPixelSettingsError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`update_pixel_settings`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub async fn get_pixel_settings(configuration: &configuration::Configuration, ) -> Result<models::GetPixelSettings200Response, Error<GetPixelSettingsError>> {

    let uri_str = format!("{}/v1/pixel-settings", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetPixelSettings200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetPixelSettings200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetPixelSettingsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Update the web pixel subscribe list configuration.
pub async fn update_pixel_settings(configuration: &configuration::Configuration, update_pixel_settings_request: models::UpdatePixelSettingsRequest) -> Result<models::GetPixelSettings200Response, Error<UpdatePixelSettingsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_update_pixel_settings_request = update_pixel_settings_request;
    ...
    req_builder = req_builder.json(&p_body_update_pixel_settings_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<UpdatePixelSettingsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
