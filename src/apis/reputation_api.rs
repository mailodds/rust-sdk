...
/// struct for typed errors of method [`get_reputation_timeline`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetReputationTimelineError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_reputation(configuration: &configuration::Configuration, period: Option<&str>) -> Result<models::GetReputation200Response, Error<GetReputationError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_period = period;

    let uri_str = format!("{}/v1/reputation", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetReputation200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetReputation200Response`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_reputation_timeline(configuration: &configuration::Configuration, period: Option<&str>) -> Result<models::GetReputationTimeline200Response, Error<GetReputationTimelineError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_period = period;

    let uri_str = format!("{}/v1/reputation/timeline", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetReputationTimeline200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetReputationTimeline200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetReputationTimelineError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
