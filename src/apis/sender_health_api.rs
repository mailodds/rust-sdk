...
/// struct for typed errors of method [`get_sender_health`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetSenderHealthError {
    Status403(models::ErrorResponse),
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`get_sender_health_trend`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetSenderHealthTrendError {
    Status403(models::ErrorResponse),
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_sender_health(configuration: &configuration::Configuration, period: Option<&str>) -> Result<models::GetSenderHealth200Response, Error<GetSenderHealthError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_period = period;

    let uri_str = format!("{}/v1/sender-health", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetSenderHealth200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetSenderHealth200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetSenderHealthError> = serde_json::from_str(&content).ok();
...
pub async fn get_sender_health_trend(configuration: &configuration::Configuration, period: Option<&str>) -> Result<models::GetSenderHealthTrend200Response, Error<GetSenderHealthTrendError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_period = period;

    let uri_str = format!("{}/v1/sender-health/trend", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetSenderHealthTrend200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetSenderHealthTrend200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetSenderHealthTrendError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
