...
/// struct for typed errors of method [`create_delegation`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreateDelegationError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`list_delegations`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListDelegationsError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`revoke_delegation`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum RevokeDelegationError {
    Status401(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}
...
pub async fn create_delegation(configuration: &configuration::Configuration, domain_id: &str) -> Result<(), Error<CreateDelegationError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_domain_id = domain_id;

    let uri_str = format!("{}/v1/sending-domains/{domain_id}/delegations", configuration.base_path, domain_id=crate::apis::urlencode(p_path_domain_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<CreateDelegationError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// List all sending delegations for a domain.
pub async fn list_delegations(configuration: &configuration::Configuration, domain_id: &str) -> Result<(), Error<ListDelegationsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_domain_id = domain_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ListDelegationsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Revoke a sending delegation, removing the delegate account permission to send.
pub async fn revoke_delegation(configuration: &configuration::Configuration, domain_id: &str, delegation_id: &str) -> Result<(), Error<RevokeDelegationError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_domain_id = domain_id;
    let p_path_delegation_id = delegation_id;

    let uri_str = format!("{}/v1/sending-domains/{domain_id}/delegations/{delegation_id}", configuration.base_path, domain_id=crate::apis::urlencode(p_path_domain_id), delegation_id=crate::apis::urlencode(p_path_delegation_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<RevokeDelegationError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
