...
pub struct AppendToContactListRequest {
    /// Validation job IDs to append from
    #[serde(rename = "source_job_ids")]
    pub source_job_ids: Vec<String>,
    /// Include catch-all emails in addition to valid ones
    #[serde(rename = "include_catch_all", skip_serializing_if = "Option::is_none")]
    pub include_catch_all: Option<bool>,
}

impl AppendToContactListRequest {
    pub fn new(source_job_ids: Vec<String>) -> AppendToContactListRequest {
        AppendToContactListRequest {
            source_job_ids,
            include_catch_all: None,
        }
    }
}
